<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Organoleptico_temp.aspx.cs" Inherits="rinya_app.Calidad.Organoleptico_temp" %>
   ...
   <script type="text/javascript"> 
       ...
       function loadTable() {
          ...
          var table = $('#tblData').DataTable({
               ...
               "bProcessing": true,
               "bServerSide": true,
               "sAjaxSource": "WebService_Organoleptico.asmx/GetTableData4",
          ...
          $("#tblData tbody").on("click", 'tr', function (e) {
              ...
              C_Datos.ID_LOTE = $(nTds[12]).text();
              C_Datos.ID_ESTADO = $(nTds[13]).text();
              C_Datos.OBS_ESTADO = $(nTds[8]).text();
             
              $('#blck_ID').hide();
              $('#blck_ID_LOTE').hide();
              C_Datos.MERMA = $(nTds[14]).text();
              for (var j = 1; j <= 65; j++){
                  $('#Blck_' + j).hide();
              ...
              $('#table-dialogo').dialog({
                  ...
                  open: function (event, ui) {
                      if (C_Datos.ARTICULO.length > 0) {
                          ...
                          $('#Texto_Lote_Interno').text($(nTds[3]).text()+ ' | N Palet ' + $(nTds[2]).text());
                          $('#Texto_Fecha_Caducidad').text($(nTds[9]).text());
                          $('#Texto_ID_LOTE').text($(nTds[12]).text());
                          $('#Texto_ID').text($(nTds[0]).text());
                          $('#text_0').val($(nTds[8]).text());
                         ...
                         // $('#ddlEstado').selectmenu().selectmenu('change', cambiarvalor($('#ddlEstado').val));
                          ...
                          $.ajax({
                              type: "POST",
                              contentType: "application/json; charset=utf-8",
                              url: "WebService_Organoleptico.asmx/GetCaracteristicas_articulo2",
...
<th> <p>Hasta: <input type="text" id="datepicker2" class="form-control" style="width: 98px"></p></th>
             <td>&nbsp;&nbsp; <p>&nbsp;&nbsp;<input id="btnToda" type="button" Value="Obtener Matriculas" class="btn btn-default" onclick="loadTable();"/></p></td>
